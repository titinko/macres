...
fft_plan fft_plan_dft_1d(int n, fft_complex *in, fft_complex *out, int sign, unsigned int flags)
{
	...
	output.sign		= sign;
	output.flags	= flags;
	// In OSX already 16-byte aligned
	output.input = (double *)malloc(sizeof(double) * n * 2);
	//_aligned_malloc((void **)&output.input, sizeof(double) * n * 2, 16);
...
fft_plan fft_plan_dft_c2r_1d(int n, fft_complex *in, double *out, unsigned int flags)
{
	...
	output.flags	= flags;
	output.input = (double *)malloc(sizeof(double) * n); // In OSX already 16-byte aligned
	//_aligned_malloc((void **)&output.input, sizeof(double) * n, 16);
	output.ip		= (int *)malloc(sizeof(int) * n);
	output.w		= (double *)malloc(sizeof(double) * n * 5 / 4);
